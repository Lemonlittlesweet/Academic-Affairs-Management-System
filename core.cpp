...
void core::stu_former() {
	...
	int f;
	f = 0;
	for (int i = searchc-1; i>=0; i--)//在表里找到学生选的课程
	{
		if (!strcmp(ID.toUtf8().data(), ::list.s_course[i].stu_id))
		{
			flag1 = 1;
			for (int j = searcht-1; j >=0; j--)//在老师授课表里找到授课的老师
			{
...
void core::stu_latter() {
	...
	int f;
	f = 0;
	for (int i = searchc+1; i < ::list.num; i++)//在表里找到学生选的课程
	{
		if (!strcmp(ID.toUtf8().data(), ::list.s_course[i].stu_id))
		{
			flag1 = 1;
			for (int j = searcht+1; j < ::list.tc_n; j++)//在老师授课表里找到授课的老师
			{
				if (!strcmp(::list.s_course[i].sub_id, ::list.t_course[j].sub_id))
				{
					for (int k = 0; k < ::list.t_n; k++)
