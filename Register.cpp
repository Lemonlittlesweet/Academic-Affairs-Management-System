...
void Register::PushFinishButton() {
	if (!(ui.password->text().isEmpty() || ui.repw->text().isEmpty() || ui.user->text().isEmpty() || ui.name->text().isEmpty())) {
		if (ui.password->text() == ui.repw->text())
		{
			if (::list.regist_teacher(ui.user->text().toUtf8().data(), ui.name->text().toUtf8().data(), ui.password->text().toUtf8().data()) >= 0)
			{
